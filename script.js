// Sample product data
const products = {
  men: {
    jacket: [
      {
        img: "./assets/image/men/jacket/jacket--01.png",
        name: "NightRider",
        price: 1999,
        color: "Black",
        material: "Pure Leather",
        desc: "Premium leather jacket with a bold biker-inspired silhouette.",
      },
      {
        img: "./assets/image/men/jacket/jacket--02.png",
        name: "BlueForge",
        price: 1499,
        color: "Blue",
        material: "Denim 350 GSM",
        desc: "Classic denim jacket built for effortless everyday styling.",
      },
      {
        img: "./assets/image/men/jacket/jacket--03.png",
        name: "FrostShield",
        price: 2499,
        color: "Grey",
        material: "Winter Fleece",
        desc: "Heavy insulated jacket designed to lock in warmth during cold days.",
      },
      {
        img: "./assets/image/men/jacket/jacket--04.png",
        name: "Glacier Core",
        price: 2399,
        color: "Blue",
        material: "Winter Fleece",
        desc: "Cold-weather essential with a structured fit and strong insulation.",
      },
      {
        img: "./assets/image/men/jacket/jacket--05.png",
        name: "UrbanTherm",
        price: 2199,
        color: "Black",
        material: "Polyester Shell",
        desc: "Lightweight outerwear that delivers warmth without bulk.",
      },
      {
        img: "./assets/image/men/jacket/jacket--06.png",
        name: "Arctic Field",
        price: 2299,
        color: "Olive",
        material: "Thermal Lining",
        desc: "Built for extreme winters with high heat retention and comfort.",
      },
      {
        img: "./assets/image/men/jacket/jacket--07.png",
        name: "Rustic Edge",
        price: 2499,
        color: "Brown",
        material: "Fleece 400 GSM",
        desc: "Stylish winter jacket with a rugged look and heavy fleece warmth.",
      },
      {
        img: "./assets/image/men/jacket/jacket--08.png",
        name: "CloudPuff",
        price: 1999,
        color: "Grey",
        material: "Puffer Material",
        desc: "Ultra-light puffer jacket designed for easy layering and comfort.",
      },
      {
        img: "./assets/image/men/jacket/jacket--09.png",
        name: "Polar Crest",
        price: 2599,
        color: "Navy",
        material: "Soft Shell",
        desc: "Premium soft-shell jacket with a clean finish and thermal protection.",
      },
      {
        img: "./assets/image/men/jacket/jacket--10.png",
        name: "ColdFront Pro",
        price: 2299,
        color: "Black",
        material: "Insulated",
        desc: "Reliable winter jacket crafted for harsh weather conditions.",
      },
    ],

    tshirt: [
      {
        img: "./assets/image/men/t-shirts/t-shirt-11.png",
        name: "Noir Essence",
        price: 599,
        color: "Black",
        material: "Bio-Wash Cotton 180 GSM",
        desc: "Soft bio-wash cotton t-shirt offering comfort, durability, and a smooth finish.",
      },
      {
        img: "./assets/image/men/t-shirts/t-shirt-12.png",
        name: "Ivory Mode",
        price: 499,
        color: "White",
        material: "Cotton 160 GSM",
        desc: "Lightweight cotton t-shirt designed for everyday comfort and a clean fit.",
      },
      {
        img: "./assets/image/men/t-shirts/t-shirt-13.png",
        name: "Ash Royale",
        price: 550,
        color: "Grey",
        material: "210 GSM Premium Cotton",
        desc: "Premium cotton t-shirt with a slightly heavier fabric for long-lasting wear.",
      },
      {
        img: "./assets/image/men/t-shirts/t-shirt-14.png",
        name: "Suncrest",
        price: 520,
        color: "Yellow",
        material: "Cotton",
        desc: "Comfortable cotton t-shirt with a relaxed fit for daily casual use.",
      },
      {
        img: "./assets/image/men/t-shirts/t-shirt-15.png",
        name: "Verdant Prime",
        price: 650,
        color: "Green",
        material: "Heavy 250 GSM",
        desc: "Heavyweight cotton t-shirt providing a structured fit and premium feel.",
      },
      {
        img: "./assets/image/men/t-shirts/t-shirt-16.png",
        name: "Azure Lane",
        price: 499,
        color: "Blue",
        material: "Cotton",
        desc: "Classic cotton t-shirt with breathable fabric for all-day comfort.",
      },
      {
        img: "./assets/image/men/t-shirts/t-shirt-17.png",
        name: "Pureline",
        price: 599,
        color: "White",
        material: "180 GSM Cotton",
        desc: "High-quality cotton t-shirt with a soft texture and comfortable fit.",
      },
      {
        img: "./assets/image/men/t-shirts/t-shirt-18.png",
        name: "Obsidian Wear",
        price: 579,
        color: "Black",
        material: "Cotton",
        desc: "Durable cotton t-shirt designed for regular wear and easy maintenance.",
      },
      {
        img: "./assets/image/men/t-shirts/t-shirt-19.png",
        name: "Terra Luxe",
        price: 560,
        color: "Brown",
        material: "190 GSM",
        desc: "Well-balanced cotton t-shirt with reinforced stitching for added durability.",
      },
      {
        img: "./assets/image/men/t-shirts/t-shirt-20.png",
        name: "Greyline Studio",
        price: 570,
        color: "Grey",
        material: "Cotton Blend",
        desc: "Cotton-blend t-shirt offering a smooth feel and comfortable everyday fit.",
      },
    ],

    jeans: [
      {
        img: "./assets/image/men/jeans/jeans-01.png",
        name: "Blue Jeans",
        price: 1199,
        color: "Blue",
        material: "Stretch Denim",
        desc: "Slim fit stretchable denim.",
      },
      {
        img: "./assets/image/men/jeans/jeans-02.png",
        name: "Slim Fit Jeans",
        price: 1299,
        color: "Navy",
        material: "Denim",
        desc: "Smart-fit premium denim.",
      },
      {
        img: "./assets/image/men/jeans/jeans-03.png",
        name: "Slim Fit Jeans",
        price: 1499,
        color: "Light Blue",
        material: "Washed Denim",
        desc: "Washed denim for a modern look.",
      },
      {
        img: "./assets/image/men/jeans/jeans-04.png",
        name: "Slim Fit Jeans",
        price: 1399,
        color: "Blue",
        material: "Soft Denim",
        desc: "Stylish faded jeans.",
      },
      {
        img: "./assets/image/men/jeans/jeans-05.png",
        name: "Slim Fit Jeans",
        price: 1199,
        color: "Black",
        material: "Denim",
        desc: "Casual black jeans.",
      },
      {
        img: "./assets/image/men/jeans/jeans-06.png",
        name: "Slim Fit Jeans",
        price: 1299,
        color: "Grey",
        material: "Stretch Denim",
        desc: "Flexible and durable denim.",
      },
      {
        img: "./assets/image/men/jeans/jeans-07.png",
        name: "Slim Fit Jeans",
        price: 1499,
        color: "Blue",
        material: "Denim 12oz",
        desc: "Strong and durable jeans.",
      },
      {
        img: "./assets/image/men/jeans/jeans-08.png",
        name: "Slim Fit Jeans",
        price: 1399,
        color: "Black",
        material: "Soft Denim",
        desc: "Smooth texture quality.",
      },
      {
        img: "./assets/image/men/jeans/jeans-09.png",
        name: "Slim Fit Jeans",
        price: 1499,
        color: "Navy",
        material: "Stretchable Denim",
        desc: "Premium stretch denim.",
      },
      {
        img: "./assets/image/men/jeans/jeans-10.png",
        name: "Slim Fit Jeans",
        price: 1599,
        color: "Blue",
        material: "Heavy Denim",
        desc: "Premium classic denim.",
      },
    ],

    hoodie: [
      {
        img: "./assets/image/men/hoodies/hoodie-01.png",
        name: "Crimson Pulse",
        price: 899,
        color: "Red",
        material: "Fleece 280 GSM",
        desc: "A bold hoodie that stands out in vibrant crimson, perfect for urban adventures.",
      },
      {
        img: "./assets/image/men/hoodies/hoodie-02.png",
        name: "Golden Horizon",
        price: 999,
        color: "Golden Yellow",
        material: "Cotton Blend 300 GSM",
        desc: "Soft premium hoodie inspired by the warmth of sunset tones.",
      },
      {
        img: "./assets/image/men/hoodies/hoodie-03.png",
        name: "Midnight Drift",
        price: 950,
        color: "Charcoal",
        material: "Fleece 250 GSM",
        desc: "Sleek dark hoodie for everyday style and comfort under the night sky.",
      },
      {
        img: "./assets/image/men/hoodies/hoodie-04.png",
        name: "Emerald Grove",
        price: 1050,
        color: "Forest Green",
        material: "Organic Cotton 280 GSM",
        desc: "Earthy green hoodie with soft texture, inspired by nature’s calm.",
      },
      {
        img: "./assets/image/men/hoodies/hoodie-05.png",
        name: "Ocean Drift",
        price: 980,
        color: "Ocean Blue",
        material: "Poly-Cotton Blend 300 GSM",
        desc: "Cool blue hoodie with a relaxed fit, perfect for casual vibes.",
      },
      {
        img: "./assets/image/men/hoodies/hoodie-06.png",
        name: "Copper Ember",
        price: 970,
        color: "Rust Orange",
        material: "Fleece 270 GSM",
        desc: "Warm autumn-toned hoodie with a cozy fleece lining.",
      },
      {
        img: "./assets/image/men/hoodies/hoodie-07.png",
        name: "Slate Horizon",
        price: 940,
        color: "Slate Grey",
        material: "Cotton Fleece 260 GSM",
        desc: "Neutral grey hoodie with a modern minimalist appeal.",
      },
      {
        img: "./assets/image/men/hoodies/hoodie-08.png",
        name: "Berry Bloom",
        price: 990,
        color: "Berry Pink",
        material: "Fleece 280 GSM",
        desc: "Playful pink hoodie with a soft feel, ideal for casual outings.",
      },
      {
        img: "./assets/image/men/hoodies/hoodie-09.png",
        name: "Steel Wave",
        price: 1020,
        color: "Steel Blue",
        material: "Cotton Blend 290 GSM",
        desc: "Modern steel-blue hoodie with a comfortable, stylish fit.",
      },
      {
        img: "./assets/image/men/hoodies/hoodie-00.png",
        name: "Ivory Mist",
        price: 1090,
        color: "Ivory White",
        material: "Fleece 300 GSM",
        desc: "Elegant ivory hoodie with soft fleece, perfect for layering and style.",
      },
    ],

    shoe: [
      {
        img: "./assets/image/men/shoes/shoe-01.png",
        name: "Redline Sprint",
        price: 1599,
        color: "Red",
        material: "Mesh + Rubber",
        desc: "Lightweight sneakers designed for speed, comfort, and daily movement.",
      },
      {
        img: "./assets/image/men/shoes/shoe-02.png",
        name: "Urban Vault",
        price: 1699,
        color: "Gold",
        material: "Synthetic",
        desc: "Modern street-style sneakers built for everyday city wear.",
      },
      {
        img: "./assets/image/men/shoes/shoe-03.png",
        name: "Aurex Prime",
        price: 1799,
        color: "Gold",
        material: "Synthetic Leather",
        desc: "Premium sneakers with a bold design and smooth leather finish.",
      },
      {
        img: "./assets/image/men/shoes/shoe-04.png",
        name: "Cloudstep Flex",
        price: 1499,
        color: "Gold",
        material: "Foam Comfort",
        desc: "Ultra-comfort sneakers with soft cushioning for all-day wear.",
      },
      {
        img: "./assets/image/men/shoes/shoe-05.png",
        name: "Streetflow Lite",
        price: 1599,
        color: "Gold",
        material: "Mesh",
        desc: "Breathable lightweight sneakers made for relaxed daily styling.",
      },
      {
        img: "./assets/image/men/shoes/shoe-06.png",
        name: "AeroMove",
        price: 1499,
        color: "Gold",
        material: "Mesh",
        desc: "Airy mesh sneakers that keep your feet cool and comfortable.",
      },
      {
        img: "./assets/image/men/shoes/shoe-07.png",
        name: "ComfortX Elite",
        price: 1790,
        color: "Gold",
        material: "Soft Touch",
        desc: "Premium comfort sneakers crafted for long-lasting support.",
      },
      {
        img: "./assets/image/men/shoes/shoe-08.png",
        name: "Urban Drift",
        price: 1690,
        color: "Gold",
        material: "Synthetic Blend",
        desc: "Casual lifestyle sneakers with a clean, modern silhouette.",
      },
      {
        img: "./assets/image/men/shoes/shoe-09.png",
        name: "IronWalk",
        price: 1599,
        color: "Gold",
        material: "Rubber Sole",
        desc: "Durable sneakers built with strong grip and everyday reliability.",
      },
      {
        img: "./assets/image/men/shoes/shoe-10.png",
        name: "SoftRide Pro",
        price: 1799,
        color: "Gold",
        material: "Foam Sole",
        desc: "Super-soft cushioned sneakers made for effortless walking comfort.",
      },
    ],
  },

  women: {
    handbag: [
      {
        img: "./assets/image/women/bags/bag--01.png",
        name: "Brown Luxe Tote",
        price: 1299,
        color: "Brown",
        material: "PU Leather",
        desc: "Elegant premium handbag.",
      },
      {
        img: "./assets/image/women/bags/bag--02.png",
        name: "Midnight Sling",
        price: 899,
        color: "Black",
        material: "Soft Leather",
        desc: "Stylish sling bag for women.",
      },
      {
        img: "./assets/image/women/bags/bag-03.png",
        name: "Ivory Charm",
        price: 899,
        color: "Cream",
        material: "Leather",
        desc: "Trendy cream sling bag.",
      },
      {
        img: "./assets/image/women/bags/bag-04.png",
        name: "Pearl Loop",
        price: 950,
        color: "White",
        material: "Soft Material",
        desc: "Elegant everyday sling.",
      },
      {
        img: "./assets/image/women/bags/bag-05.png",
        name: "Classic Mocha",
        price: 999,
        color: "Brown",
        material: "Leather",
        desc: "Classic leather sling.",
      },
      {
        img: "./assets/image/women/bags/bag-06.png",
        name: "Noir Daily",
        price: 899,
        color: "Black",
        material: "PU Leather",
        desc: "Perfect daily-use sling.",
      },
      {
        img: "./assets/image/women/bags/bag-07.png",
        name: "Heritage Craft",
        price: 999,
        color: "Brown",
        material: "Pure Leather",
        desc: "Premium handcrafted sling.",
      },
      {
        img: "./assets/image/women/bags/bag-08.png",
        name: "Urban Pearl",
        price: 1050,
        color: "White",
        material: "Soft PU",
        desc: "Modern stylish sling bag.",
      },
    ],

    heels: [
      {
        img: "./assets/image/women/heels/heel-01.png",
        name: "Scarlet Flame",
        price: 1599,
        color: "Red",
        material: "Synthetic",
        desc: "Stylish red heels for parties.",
      },
      {
        img: "./assets/image/women/heels/heel-02.png",
        name: "Golden Grace",
        price: 1699,
        color: "Gold",
        material: "Shimmer Material",
        desc: "Premium golden heels.",
      },
      {
        img: "./assets/image/women/heels/heel-03.png",
        name: "Royal Glow",
        price: 1499,
        color: "Gold",
        material: "Synthetic",
        desc: "Elegant golden footwear.",
      },
      {
        img: "./assets/image/women/heels/heel-04.png",
        name: "Festive Spark",
        price: 1799,
        color: "Gold",
        material: "Shining Fabric",
        desc: "Perfect for festive wear.",
      },
      {
        img: "./assets/image/women/heels/heel-05.png",
        name: "Golden Muse",
        price: 1599,
        color: "Gold",
        material: "Soft Fabric",
        desc: "Trendy comfortable heels.",
      },
      {
        img: "./assets/image/women/heels/heel-06.png",
        name: "Urban Gold",
        price: 1690,
        color: "Gold",
        material: "Synthetic",
        desc: "Durable and stylish.",
      },
      {
        img: "./assets/image/women/heels/heel-07.png",
        name: "Silk Step",
        price: 1650,
        color: "Gold",
        material: "Synthetic",
        desc: "Comfortable stylish heels.",
      },
      {
        img: "./assets/image/women/heels/heel-08.png",
        name: "Party Gleam",
        price: 1790,
        color: "Gold",
        material: "Shiny Material",
        desc: "Party wear heels.",
      },
      {
        img: "./assets/image/women/heels/heel-09.png",
        name: "Soft Shine",
        price: 1499,
        color: "Gold",
        material: "Soft",
        desc: "Lightweight and trendy.",
      },
      {
        img: "./assets/image/women/heels/heel-10.png",
        name: "Golden Empress",
        price: 1899,
        color: "Gold",
        material: "Metallic Fabric",
        desc: "Premium designer heels.",
      },
    ],

    kurti: [
      {
        img: "./assets/image/women/kurties/kurti-01.png",
        name: "Ruby Anarkali",
        price: 899,
        color: "Red",
        material: "Cotton",
        desc: "Soft and traditional kurti.",
      },
      {
        img: "./assets/image/women/kurties/kurti-02.png",
        name: "Golden Festive",
        price: 999,
        color: "Gold",
        material: "Rayon",
        desc: "Modern festive kurti.",
      },
      {
        img: "./assets/image/women/kurties/kurti-03.png",
        name: "Pink Blossom",
        price: 1099,
        color: "Pink",
        material: "Cotton",
        desc: "Comfortable daily kurti.",
      },
      {
        img: "./assets/image/women/kurties/kurti-04.png",
        name: "Blue Elegance",
        price: 999,
        color: "Blue",
        material: "Rayon",
        desc: "Stylish party wear.",
      },
      {
        img: "./assets/image/women/kurties/kurti-05.png",
        name: "Emerald Grace",
        price: 1199,
        color: "Green",
        material: "Soft Cotton",
        desc: "Trendy ethnic kurti.",
      },
      {
        img: "./assets/image/women/kurties/kurti-06.png",
        name: "Grey Aura",
        price: 1050,
        color: "Grey",
        material: "Rayon",
        desc: "Lightweight and comfortable.",
      },
      {
        img: "./assets/image/women/kurties/kurti-07.png",
        name: "Sunlit Cotton",
        price: 1150,
        color: "Yellow",
        material: "Cotton",
        desc: "Summer-friendly kurti.",
      },
      {
        img: "./assets/image/women/kurties/kurti-08.png",
        name: "Green Harmony",
        price: 999,
        color: "Green",
        material: "Cotton",
        desc: "Elegant ethnic wear.",
      },
      {
        img: "./assets/image/women/kurties/kurti-09.png",
        name: "Earth Muse",
        price: 1099,
        color: "Brown",
        material: "Rayon",
        desc: "Soft and breathable.",
      },
      {
        img: "./assets/image/women/kurties/kurti-10.png",
        name: "Azure Flow",
        price: 990,
        color: "Blue",
        material: "Cotton Blend",
        desc: "Light and stylish kurti.",
      },
    ],

    hoodie: [
      {
        img: "./assets/image/women/hoodies/hoodie-01.png",
        name: "Crimson Cozy",
        price: 899,
        color: "Red",
        material: "Fleece",
        desc: "Warm and comfortable hoodie.",
      },
      {
        img: "./assets/image/women/hoodies/hoodie-02.png",
        name: "Golden Aura",
        price: 999,
        color: "Golden",
        material: "Fleece",
        desc: "Premium fleece hoodie.",
      },
      {
        img: "./assets/image/women/hoodies/hoodie-03.png",
        name: "Soft Luxe",
        price: 950,
        color: "Golden",
        material: "Soft Fleece",
        desc: "Cozy fashionable hoodie.",
      },
      {
        img: "./assets/image/women/hoodies/hoodie-04.png",
        name: "Warm Classic",
        price: 999,
        color: "Golden",
        material: "Fleece",
        desc: "Classic warm hoodie.",
      },
      {
        img: "./assets/image/women/hoodies/hoodie-05.png",
        name: "Winter Shield",
        price: 1090,
        color: "Golden",
        material: "Fleece",
        desc: "Best winter protection.",
      },
      {
        img: "./assets/image/women/hoodies/hoodie-06.png",
        name: "Frost Vogue",
        price: 1000,
        color: "Golden",
        material: "Fleece",
        desc: "Winter fashion hoodie.",
      },
      {
        img: "./assets/image/women/hoodies/hoodie-07.png",
        name: "Velvet Touch",
        price: 1050,
        color: "Golden",
        material: "Fleece",
        desc: "Soft & premium hoodie.",
      },
      {
        img: "./assets/image/women/hoodies/hoodie-08.png",
        name: "Daily Cozy",
        price: 950,
        color: "Golden",
        material: "Soft Fleece",
        desc: "Daily wear warm hoodie.",
      },
      {
        img: "./assets/image/women/hoodies/hoodie-09.png",
        name: "Urban Glow",
        price: 999,
        color: "Golden",
        material: "Fleece",
        desc: "Trendy hoodie.",
      },
      {
        img: "./assets/image/women/hoodies/hoodie-10.png",
        name: "Elite Warmth",
        price: 1099,
        color: "Golden",
        material: "Fleece",
        desc: "Classic premium hoodie.",
      },
    ],

    cargo: [
      {
        img: "./assets/image/women/cargos/cargo-01.png",
        name: "Midnight Utility",
        price: 1299,
        color: "Black",
        material: "Cotton",
        desc: "Trendy cargo pants for women.",
      },
      {
        img: "./assets/image/women/cargos/cargo-02.png",
        name: "Earthline Cargo",
        price: 1399,
        color: "Brown",
        material: "Soft Cotton",
        desc: "Stylish brown cargo.",
      },
      {
        img: "./assets/image/women/cargos/cargo-03.png",
        name: "Olive Edge",
        price: 1290,
        color: "Olive",
        material: "Cotton",
        desc: "Comfortable olive cargo.",
      },
      {
        img: "./assets/image/women/cargos/cargo-04.png",
        name: "Urban Slim",
        price: 1390,
        color: "Black",
        material: "Cotton",
        desc: "Modern slim-fit cargo.",
      },
      {
        img: "./assets/image/women/cargos/cargo-05.png",
        name: "Grey Motion",
        price: 1499,
        color: "Grey",
        material: "Soft Fabric",
        desc: "Trendy grey cargo.",
      },
      {
        img: "./assets/image/women/cargos/cargo-06.png",
        name: "Evergreen Flow",
        price: 1299,
        color: "Green",
        material: "Cotton",
        desc: "Stylish evergreen cargo.",
      },
      {
        img: "./assets/image/women/cargos/cargo-07.png",
        name: "Cream Avenue",
        price: 1499,
        color: "Cream",
        material: "Cotton",
        desc: "Trendy cream cargo pants.",
      },
      {
        img: "./assets/image/women/cargos/cargo-08.png",
        name: "Olive Street",
        price: 1390,
        color: "Olive",
        material: "Cotton",
        desc: "Comfort fit olive cargo.",
      },
      {
        img: "./assets/image/women/cargos/cargo-09.png",
        name: "Denim Utility",
        price: 1490,
        color: "Blue",
        material: "Denim Cargo",
        desc: "Modern denim cargo pants.",
      },
      {
        img: "./assets/image/women/cargos/cargo-10.png",
        name: "Black Crest",
        price: 1590,
        color: "Black",
        material: "Soft Cotton",
        desc: "Premium cargo pants.",
      },
    ],
  },
};

// GET ELEMENTS
const productModal = document.getElementById("productModal");
const closeBtn = productModal.querySelector(".custom-close");

// OPEN MODAL FUNCTION
document.addEventListener("click", (e) => {
  const card = e.target.closest(".product-card");
  if (!card) return;

  const prod = JSON.parse(card.dataset.product);

  document.getElementById("modalImg").src = prod.img;
  document.getElementById("modalTitle").textContent = prod.name;
  // document.getElementById("modalGsm").textContent =
  //   prod.gsm ?? "275 GSM Jersey";
  document.getElementById("modalColor").textContent = prod.color ?? "White";
  document.getElementById("modalPrice").textContent = prod.price
    ? `$${prod.price}`
    : "$35";
  document.getElementById("modalDesc").textContent =
    prod.desc ?? "This is one of our best-selling products.";

  // Show modal
  productModal.classList.add("show");
});

// CLOSE MODAL
closeBtn.addEventListener("click", () => {
  productModal.classList.remove("show");
});

// OPTIONAL: Close when clicking outside modal content
productModal.addEventListener("click", (e) => {
  if (e.target === productModal) {
    productModal.classList.remove("show");
  }
});

//

//
// When switching tabs (MEN <-> WOMEN), clear product list + reset modal
document.querySelectorAll('[data-bs-toggle="tab"]').forEach((tab) => {
  tab.addEventListener("shown.bs.tab", () => {
    // CLEAR PRODUCT LIST
    document.getElementById("product-list").innerHTML = "";
    document.getElementById("product-title").textContent = "";

    // RESET MODAL CONTENT
    document.getElementById("modalImg").src = "";
    document.getElementById("modalTitle").textContent = "";
    document.getElementById("modalGsm").textContent = "";
    document.getElementById("modalColor").textContent = "";
    document.getElementById("modalPrice").textContent = "";
    document.getElementById("modalDesc").textContent = "";

    // FORCE CLOSE THE MODAL if open
    productModal.classList.remove("show");
  });
});

//

// Accordion functionality
const accordionItems = document.querySelectorAll(".accordion-item");

accordionItems.forEach((item) => {
  item.addEventListener("click", () => {
    // Toggle active class
    item.classList.toggle("active");

    // If you want only one open at a time, uncomment below:
    // accordionItems.forEach(other => {
    //   if(other !== item) other.classList.remove("active");
    // });
  });
});

// for button add to cart and buy now
const addToCartBtn = document.getElementById("addToCartBtn");
const buyNowBtn = document.getElementById("buyNowBtn");

addToCartBtn.addEventListener("click", () => {
  alert(`${document.getElementById("modalTitle").textContent} added to cart!`);
  // Here you can add real cart functionality
});

buyNowBtn.addEventListener("click", () => {
  alert(`Proceed to buy ${document.getElementById("modalTitle").textContent}`);
  // Redirect to checkout page if needed
});

//

// Select menu items
document.querySelectorAll(".menu-item").forEach((item) => {
  item.addEventListener("click", () => {
    const category = item.querySelector("p").textContent.toLowerCase();
    const isMenTab = item.closest("#home-tab-pane") !== null;
    const gender = isMenTab ? "men" : "women";

    const productList = document.getElementById("product-list");
    const productTitle = document.getElementById("product-title");

    productList.innerHTML = ""; // Clear previous items
    productTitle.textContent =
      category.charAt(0).toUpperCase() + category.slice(1);

    const selectedProducts = products[gender][category] || [];

    if (selectedProducts.length === 0) {
      productList.innerHTML = "<p>No products found.</p>";
      return;
    }

    selectedProducts.forEach((prod) => {
      const col = document.createElement("div");

      // Bootstrap grid columns (Bento responsive)
      col.className = "col-6 col-md-4 col-lg-3 p-2";

      col.innerHTML = `
  <div class="card product-card h-150 p-2 shadow-sm rounded-4 overflow-hidden"
       data-product='${JSON.stringify(prod)}'>
      <img src="${
        prod.img
      }" class="card-img-top object-fit-cover" style="height: 250px;">
      <div class="card-body text-center">
        <p class="card-text fw-semibold m-0">${prod.name}</p>
      </div>
  </div>
`;

      document.getElementById("product-list").appendChild(col);
    });
  });
});
